...
import { FileText, Github, Home, Linkedin, Mail } from "lucide-react";
...
export default function GlassDock({ active = "home" }: { active?: DockActive }) {
  return (
    <nav
      className="fixed bottom-5 left-1/2 z-[60] flex -translate-x-1/2 items-stretch gap-0.5 rounded-[1.25rem] border border-white/50 bg-white/30 px-1.5 py-2 shadow-[0_12px_40px_rgb(0_0_0_/_0.14)] backdrop-blur-2xl backdrop-saturate-150 sm:bottom-6 sm:gap-1 sm:rounded-2xl sm:px-2 sm:py-2.5"
    ...
    >
      ...
      <DockLink href="https://github.com" label="Resume">
        <Github className="h-5 w-5 sm:h-[22px] sm:w-[22px]" strokeWidth={1.85} />
      </DockLink>
      <DockLink href="https://linkedin.com" label="LinkedIn">
        <Linkedin className="h-5 w-5 sm:h-[22px] sm:w-[22px]" strokeWidth={1.85} />
      </DockLink>
      <DockLink href="mailto:hello@example.com" label="Hire Me">
